...
public class MyInflater : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length / 3; ++i_tri)
        {
            ...
            {
                int i_vtx = node2vtx[i_node];
                bsm.AddBlockAt(i_vtx, num_vtx, new float3x3(
                    grad_volume[i_node].x, 0, 0,
                    grad_volume[i_node].y, 0, 0,
                    grad_volume[i_node].z, 0, 0
                ));
                bsm.AddBlockAt(num_vtx, i_vtx, new float3x3(
                    grad_volume[i_node].x, grad_volume[i_node].y, grad_volume[i_node].z,
                    0, 0, 0,
                    0, 0, 0
                ));
                rhs_vector[i_vtx] += lambda * grad_volume[i_node];
            }
