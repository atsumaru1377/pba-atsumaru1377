fn problem0() -> anyhow::Result<()> {
    ...
    for i_frame in 0..num_frame {
        ...
        // position in the pixel coordinate
        let p1 = [p0[0] + 100f32 * radian.cos(), p0[1] - 100f32 * radian.sin()];
...
fn problem1() -> anyhow::Result<()> {
    ...
    for i_frame in 0..num_frame {
        ...
        for i in 0..(trajectory.len() - 1) {
            let p_i0 = trajectory[i];
            let p_i1 = trajectory[i + 1];
            del_canvas::rasterize::line::draw_dda(&mut canvas.data, canvas.width, &p_i0, &p_i1, 3);
        }
        // ---------------------
